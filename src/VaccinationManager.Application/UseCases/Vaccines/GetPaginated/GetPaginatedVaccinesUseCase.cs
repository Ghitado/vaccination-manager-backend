...
public class GetPaginatedVaccinesUseCase : IGetPaginatedVaccinesUseCase
{
	private readonly IVaccineRepository _vaccineRepository;

	public GetPaginatedVaccinesUseCase(IVaccineRepository vaccineRepository)
	{
		_vaccineRepository = vaccineRepository;	
	}

	public async Task<PaginatedResult<VaccineResponse>> Execute(int pageNumber = 1, int pageSize = 10)
	{
		if (pageNumber is < 1)
		...
		var vaccines = await _vaccineRepository.GetPaginated(pageNumber, pageSize);
